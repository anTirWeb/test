...
<div class="event-form-when" id="em-form-when">
	<p class="em-date-range">
		...
		<?php 
		$myStart = date_i18n('m/d/Y', $EM_Event->start);
		$myEnd = date_i18n('m/d/Y', $EM_Event->end);
		echo "hello";
		?>
		<input class="em-date-input" type="hidden" name="event_start_date" value="<?php $EM_Event->start()->getDate('m/d/Y');; ?>" />
		<?php _e('to','events-manager'); ?>
		<input class="em-date-end em-date-input-loc" type="text" />
		<input class="em-date-input" type="hidden" name="event_end_date" value="<?php $EM_Event->end()->getDate('m/d/Y');; ?>" />
	</p>
