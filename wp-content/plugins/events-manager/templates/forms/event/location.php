				...
				<select name="location_id" id='location-select-id' size="1">  
					...
					foreach($locations as $EM_Location) {
						...
						if( $selected ) $found_location = true;
				   		?>          
				    	<option value="<?php echo esc_attr($EM_Location->location_id) ?>" title="<?php echo esc_attr("{$EM_Location->location_latitude},{$EM_Location->location_longitude}"); ?>" <?php echo $selected ?>><?php echo esc_html($EM_Location->location_name) . esc_html($EM_Location->location_town) . ', ' . esc_html($EM_Location->location_state); ?></option>
				    	<?php
					}
					if( empty($found_location) && !empty($EM_Event->location_id) ){
						...
						if( $EM_Location->post_id ){
							?>
					    	<option value="<?php echo esc_attr($EM_Location->location_id) ?>" title="<?php echo esc_attr("{$EM_Location->location_latitude},{$EM_Location->location_longitude}"); ?>" selected="selected"><?php echo esc_html($EM_Location->location_name) . esc_html($EM_Location->location_town) . ', ' . esc_html($EM_Location->location_state); ?></option>
					    	<?php
						}
					}
					?>
