...
class EM_Event extends EM_Object{
	...
	function output($format, $target="html") {	
		...
		$format = do_shortcode($format, 1); //parse shortcodes in content first, so that formats within shortcodes are parsed properly, however uncommenting this will break shortcode containing placeholders for arguments
	 	$event_string = $format;
			...
			if( count($conditionals[0]) > 0 ){
				...
				foreach($conditionals[1] as $key => $condition){
					...
					if ($condition == 'has_bookings') {
					    ...
					    //event has this tag
					    //echo "found a tag! $tag_match[1]<br/>";
					...
					//echo "Event string: $event_string<br/>";	
				}
			}
	 	}
